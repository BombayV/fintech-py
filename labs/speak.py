...
# 4. Use print and a built-in method to print out Mr. Schmidt's name, but with all the instances of the letter i
# removed or deleted.
print(name.replace("i", ""))

# 5. Use print and a built-in function to find out how many characters are in Mr. Schmidt's name. # NOTE that since
# this is a function and not a method, the best solution won't be on the string methods page - you'll need to Google
# this one.
print(len(name))

# 6. Use a built-in method to replace every letter J with the letter G instead. Bonus point if you can say the result
...
# 8. Try to print the name as four separate strings by using a method that will split it up like this: ["John",
...
print(name[0:10].upper())

# 11. CHALLENGE: Try to print out the string "tdimhcS remiehelgniJ bocaJ nhoJ" (which is the name backwards)
print(name[::-1])

# 12. CHALLENGE: Chain two methods together to print out the string "TDIMHCS REMIEHELGNIJ BOCAJ NHOJ", (which is the
# name in uppercase and backwards).
print(name.upper()[::-1])
